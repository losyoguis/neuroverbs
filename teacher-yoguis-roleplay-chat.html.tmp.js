const $ = (s)=>document.querySelector(s);
  ...
  function renderChat(convo){
    ...
    $("#chatMeta").textContent = (convo.status==="done" ? "Concluida" : "Activa") + " • " + escapeHtml(convo.cat||"") + " • " + timeAgo(convo.updatedAt||convo.createdAt);

    const log = $("#chatLog");
    log.innerHTML = (convo.messages||[]).map(m=>{
      const isUser = m.role==="user";
      const cls = isUser ? "tyMsg tyMsgUser" : "tyMsg tyMsgBot";
      const label = isUser ? "Tú" : "Teacher Yoguis";
      return `
        <div class="${cls}">
  ...
  function buildApiMessages(convo){
    ...
    const ctx = `Role play: "${title}"${cat?` (categoría: ${cat})`:``}. Tú eres el otro personaje. Mantén el escenario realista, corrige si hace falta y termina con una pregunta para continuar.`;
  ...
  async function send(){
    ...
    if((convo.status||"open")==="done"){
      alert("Este role play ya está concluido. Puedes crear uno nuevo desde Role Play.");
      return;
    }
