const $ = (s)=>document.querySelector(s);
  ...
  function buildApiMessages(convo){
    ...
    const ctx = topic
      ? `Tema: "${topic}". Guíame en speaking: corrige gramática, sugiere frases naturales y termina con una pregunta para continuar.`
      : "Guíame en speaking: corrige gramática, sugiere frases naturales y termina con una pregunta para continuar.";
  ...
  async function send(){
    ...
    if((convo.status||"open")==="done"){
      alert("Esta conversación ya está concluida. Puedes crear una nueva desde Speaking.");
      return;
    }
    ...
    try{
      ...
      const data = await IA.chat({ mode:"speaking", messages: buildApiMessages(convo) });
...
function markDone(){
  ...
  function initVoice(){
    ...
    if(!SR){
      btn.disabled = true;
      $("#voiceNote").textContent = "Tu navegador no soporta dictado. Puedes escribir normalmente.";
      return;
    }
    ...
    rec.onend = ()=>{
      ...
      btn.classList.remove("isListening");
    };
    ...
    btn.addEventListener("click", ()=>{
      if(!rec) return;
      if(!listening){
        finalText = ($("#chatText").value||"").trim() ? ($("#chatText").value||"").trim() + " " : "";
        listening = true;
        btn.textContent = "Detener";
        btn.classList.add("isListening");
  ...
  (function init(){
    const id = qs("id");
    if(!id){ location.href = "teacher-yoguis-speaking.html"; return; }

    const arr = loadConvos();
    const convo = arr.find(x=>x.id===id);
    if(!convo){ location.href = "teacher-yoguis-speaking.html"; return; }
